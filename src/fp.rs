...
pub use ops::{FieldOps, FieldParameters, Word};
...
impl_field_ops_one_word!(FP32, u32, u64);
...
impl_field_ops_one_word!(FP64, u64, u128);
...
impl_field_ops_two_words!(FP128, u128);
...
pub(crate) mod tests {
    ...
    use crate::fp::{log2, FieldOps, Word, FP128, FP32, FP64, MAX_ROOTS};
    ...
    where
        T: FieldOps<W>,
        W: Word + AsPrimitive<u128> + ToBigInt + for<'a> TryFrom<&'a BigInt>,
        for<'a> <W as TryFrom<&'a BigInt>>::Error: Debug,
    {
        ...
        fn check_generator(&self) {
            assert_eq!(
                T::residue(T::pow(T::G, W::ONE << (self.expected_log2_order as u32))),
                W::ONE
            );
            assert_ne!(
                T::residue(T::pow(
                    T::G,
                    W::ONE << ((self.expected_log2_order as u32) / 2)
                )),
                W::ONE
            );
        }

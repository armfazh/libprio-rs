// Expander for derive bytes with uniform distribution.
//
// Code adapted from:
// https://github.com/armfazh/h2c-rust-ref
// https://github.com/armfazh/h2c-rust-ref/blob/master/src/expander/mod.rs
...
pub(super) struct ExpanderXof<T: Update + Clone + ExtendableOutput> {
    ...
    pub(super) k: usize,
    pub(super) dst_prime: RefCell<Option<Vec<u8>>>,
}
...
pub fn get_expander(id: ExpID, _dst: &[u8], k: usize) -> Box<dyn Expander> {
    let dst_prime = RefCell::new(None);
    let dst = _dst.to_vec();

...
/// `Word` is the datatype for storing and operating with field elements.
///
...
pub trait Word:
    ...
    + ConstOne
{
    type Carry: AsPrimitive<Self> + From<bool>;
}
...
pub trait FieldParameters<W: Word> {
    /// The prime modulus `p`.
    const PRIME: W;
    /// Refers to the base used for integer arithmetic operations.
    /// So `LOG2_BASE ≤ 64` as the processor has at most a 64-bit
    /// integer multiplier.
    const LOG2_BASE: usize;
    /// `mu = -p^(-1) mod 2^LOG2_BASE`.
    const MU: W;
    /// Let R be the radix used for Montgomery arithmetic,
    /// then `R = 2^Self::LOG2_RADIX`.
...
/// `FieldOps` provides the arithmetic operations over GF(p).
///
/// The multiplication method is not provided since it admits different
/// implementations.
pub trait FieldOps<W: Word>: FieldParameters<W> {
    ...
    fn add(x: W, y: W) -> W {
        ...
        let (_s1, b1) = W::Carry::from(carry)
            .as_()
            .overflowing_sub(&W::Carry::from(b0).as_());
        // if b1 == 1: return z
        // else:       return s0
        let mask = W::ZERO.wrapping_sub(&W::Carry::from(b1).as_());
        (z & mask) | (s0 & !mask)
    }
    ...
    fn sub(x: W, y: W) -> W {
        ...
        let m = W::ZERO.wrapping_sub(&W::Carry::from(b0).as_());
        //      z0
        // +     p
        // ========
        //   s1,s0
        z0.wrapping_add(&(m & Self::PRIME))
        // z0 + (m & self.p)
        // if b1 == 1: return s0
        // else:       return z0
    }
...
/// `FieldMulOpsOneWord` implements prime field multiplication.
///
/// The implementation assumes that the modulus `p` fits in one word of
/// 'W' bits. Thus the product of two integers must fit in a datatype
/// (`Self::DoubleWord`) of exactly `2*W` bits.
pub(crate) trait FieldMulOpsOneWord<W>: FieldParameters<W>
where
    W: Word + AsPrimitive<Self::DoubleWord>,
{
    ...
    fn mul(x: W, y: W) -> W {
        let hi_lo = |v: Self::DoubleWord| ((v >> W::BITS).as_(), v.as_());
        ...
        let (cc, z) =
            hi_lo(z1.as_() + r1.as_() + <Self::DoubleWord as Word>::Carry::from(carry).as_());
        ...
        // b1,s1,s0
        let (s0, b0) = z.overflowing_sub(&Self::PRIME);
        let (_s1, b1) = cc.overflowing_sub(&W::Carry::from(b0).as_());
        // if b1 == 1: return z
        // else:       return s0
        let mask = W::ZERO.wrapping_sub(&W::Carry::from(b1).as_());
        (z & mask) | (s0 & !mask)
...
/// `FieldMulOpsTwoWords` implements prime field multiplication.
///
/// The implementation assumes that the product of two integers fits in
/// a `W`-bit integer.
pub(crate) trait FieldMulOpsTwoWords<W: Word>: FieldParameters<W> {
    /// Multiplication of field elements in the Montgomery domain.
    /// This uses the Montgomery's [REDC algorithm][montgomery].
    /// The result will be in [0, p).
    ///
    ...
    fn mul(x: W, y: W) -> W {
        ...
        // ===========
        // z3,z2,z1, 0
        let mut w = low(Self::MU.wrapping_mul(&z0));
...
/// `impl_field_ops_one_word` helper to implement prime field operations.
///
/// The implementation assumes that the modulus `p` fits in one word of
/// 'W' bits, and that the product of two `W`-bit integers fits in a
/// datatype (`Self::DoubleWord`) of exactly `2*W` bits.
macro_rules! impl_field_ops_one_word {
    ($struct_name:ident, $W:ty, $W2:ty) => {
        const _: () = assert!(<$W2>::BITS == 2 * <$W>::BITS);
        impl $crate::fp::ops::FieldMulOpsOneWord<$W> for $struct_name {
            type DoubleWord = $W2;
        }
        impl $crate::fp::ops::FieldOps<$W> for $struct_name {
            #[inline(always)]
            fn mul(x: $W, y: $W) -> $W {
                <Self as $crate::fp::ops::FieldMulOpsOneWord<_>>::mul(x, y)
            }
        }
    };
}

/// `impl_field_ops_two_words` helper to implement prime field operations.
///
/// The implementation assumes that the modulus `p` fits in one word of
/// `W` bits, but the product of two `W`-bit integers
macro_rules! impl_field_ops_two_words {
    ($struct_name:ident, $W:ty) => {
        impl $crate::fp::ops::FieldMulOpsTwoWords<$W> for $struct_name {}
        impl $crate::fp::ops::FieldOps<$W> for $struct_name {
            #[inline(always)]
            fn mul(x: $W, y: $W) -> $W {
                <Self as $crate::fp::ops::FieldMulOpsTwoWords<_>>::mul(x, y)
            }
        }
    };
}

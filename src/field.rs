...
use crate::{
    ...
    prng::{Prng, Prng2, Prng3, PrngError},
    vdaf::suite::Suite,
};
use serde::{
    ...
    + 'static // NOTE This bound is needed for downcasting a `dyn Gadget<F>>` to a concrete type.
{
    ...
    /// Interprets bytes as a seed to derive a field element.
    fn hash_to_field(bytes: &[u8]) -> Result<Self, FieldError>;
...
macro_rules! make_field {
    (
        ...
        impl $elem {
            ...
            fn hash_to_field(bytes: &[u8]) -> Result<Self, FieldError> {
                let x = $fp.hash_to_field(bytes,1);
                Ok(Self($fp.elem(x[0])))
            }
        }
        ...
        impl FieldElement for $elem {
            ...
            fn from_be_bytes(bytes: &[u8]) -> Result<Self, FieldError> {
                $elem::from_be_bytes(bytes)
            }

            fn hash_to_field(bytes: &[u8]) -> Result<Self, FieldError>{
                $elem::hash_to_field(bytes)
            }
...
/// Generate a vector of uniform random field elements.
pub fn random_vector2<F: FieldElement>(len: usize) -> Result<Vec<F>, PrngError> {
    Ok(Prng2::generate(Suite::Aes128CtrHmacSha256)?
        .take(len)
        .collect())
}

/// Generate a vector of uniform random field elements.
pub fn random_vector3<F: FieldElement>(len: usize) -> Result<Vec<F>, PrngError> {
    Ok(Prng3::generate(Suite::Aes128CtrHmacSha256)?
        .take(len)
        .collect())
}

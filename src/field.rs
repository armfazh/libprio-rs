...
macro_rules! make_field {
    (
        ...
        impl FieldElement for $elem {
            ...
            fn hash_to_field(bytes: &[u8], count:usize) -> Result<Vec<Self>, FieldError>{
                $elem::hash_to_field(bytes, count)
            }
...
/// Generates `len` field elements from a byte stream provided using pad and reduce method. (simplified)
pub fn random_vector_pad_then_reduce_simpler<F: FieldElement>(
    len: usize,
) -> Result<Vec<F>, PrngError> {
    let key = Key::generate(Suite::Aes128CtrHmacSha256).unwrap();
    let mut key_stream = KeyStream::from_key(&key);
    ...
    Ok(buf
        ...
        .map(|chunk| F::from_be_bytes_simpler(chunk).unwrap())
        .collect())
}

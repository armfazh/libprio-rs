    ...
    + 'static // NOTE This bound is needed for downcasting a `dyn Gadget<F>>` to a concrete type.
{
    ...
    /// Interprets bytes as a big-endian number and reduces modulo the order of the field.
...
macro_rules! make_field {
    (
        ...
        impl $elem {
            ...
            fn from_be_bytes(bytes: &[u8;2*Self::ENCODED_SIZE]) -> Result<Self, FieldError>{
                const U64SIZE: usize = std::mem::size_of::<u64>();
                const U64_WORDS: usize = ($fp.bits + 63) / 64;
                let mut x = [0u64; 2*U64_WORDS];
            ...
            fn add<const N: usize>(x: &[u64;N], y: &[u64;N]) -> Vec<u64> {
                let mut z = Vec::<u64>::with_capacity(N + 1);
                let mut ci = false;
            ...
            fn sub<const N: usize>(x: &[u64;N], y: &[u64;N]) -> (Vec<u64>, bool) {
                 let mut z = Vec::<u64>::with_capacity(N + 1);
                let mut bi = false;
                for (xi, yi) in x.iter().zip(y.iter()) {
            ...
            // fn barret<const _2K: usize>(x: [u64;_2K]) -> Vec<u64> {
            fn barret(x: [u64;2*(($fp.bits + 63) / 64)]) -> Vec<u64> {
                let lo64 = |v: u128| -> u128 { v & ((1 << 64) - 1) };
                let hi64 = |v: u128| -> u128 { v >> 64 };
                ...
                const K: usize = ($fp.bits + 63) / 64;

                let mut buffer0 = [0u64; 8];
                let mut buffer1 = [0u64; 8];
                ...
                let r2 = &r2m[..K + 1];
                let (mut r, is_r_neg) = Self::sub::<{K + 1}>(r1.try_into().unwrap(), r2.try_into().unwrap());
                strip(&mut r, K + 1);

                if is_r_neg {
                    let mut bk1 = [0u64; K + 1 + 1];
                    bk1[K + 1] = 1;
                    r = Self::add::<{K + 1 + 1}>(&r.try_into().unwrap(), &bk1.try_into().unwrap()); // r += b^(k+1)
                    r.resize(K + 1, 0);
                }

                loop {
                    let (mut r1, is_neg) = Self::sub::<{K + 1}>(&r.clone().try_into().unwrap(), &p64[..K + 1].try_into().unwrap());
                    if is_neg {
                        strip(&mut r, K);
                        break;
                    } else {
        ...
        impl FieldElement for $elem {
            ...
            fn from_be_bytes(bytes: &[u8]) -> Result<Self, FieldError> {
                $elem::from_be_bytes(bytes.try_into().unwrap())
            }
